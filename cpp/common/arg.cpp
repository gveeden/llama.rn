...
static void parse_tensor_buffer_overrides(const std::string & value, std::vector<llama_model_tensor_buft_override> & overrides) {
    std::map<std::string, lm_ggml_backend_buffer_type_t> buft_list;
    for (size_t i = 0; i < lm_ggml_backend_dev_count(); ++i) {
        ...
        if (buft) {
            buft_list[lm_ggml_backend_buft_name(buft)] = buft;
        }
    }
    ...
    for (const auto & override : string_split<std::string>(value, ',')) {
        ...
        if (buft_list.find(buffer_type) == buft_list.end()) {
            ...
            for (const auto & it : buft_list) {
                printf("  %s\n", lm_ggml_backend_buft_name(it.second));
            }
...
static bool common_params_handle_remote_preset(common_params & params, llama_example ex) {
    LM_GGML_ASSERT(!params.model.hf_repo.empty());
...
static std::string get_all_kv_cache_types() {
    ...
    for (const auto & type : kv_cache_types) {
        msg << lm_ggml_type_name(type) << (&type == &kv_cache_types.back() ? "" : ", ");
    }
    return msg.str();
}
...
static std::vector<lm_ggml_backend_dev_t> parse_device_list(const std::string & value) {
    ...
    if (dev_names.size() == 1 && dev_names[0] == "none") {
        ...
        for (const auto & device : dev_names) {
            auto * dev = lm_ggml_backend_dev_by_name(device.c_str());
            if (!dev || lm_ggml_backend_dev_type(dev) == LM_GGML_BACKEND_DEVICE_TYPE_CPU) {
...
static void add_rpc_devices(const std::string & servers) {
    ...
    typedef lm_ggml_backend_reg_t (*lm_ggml_backend_rpc_add_server_t)(const char * endpoint);
    lm_ggml_backend_rpc_add_server_t lm_ggml_backend_rpc_add_server_fn = (lm_ggml_backend_rpc_add_server_t) lm_ggml_backend_reg_get_proc_address(rpc_reg, "lm_ggml_backend_rpc_add_server");
    if (!lm_ggml_backend_rpc_add_server_fn) {
        throw std::invalid_argument("failed to find RPC add server function");
    }
    for (const auto & server : rpc_servers) {
        auto reg = lm_ggml_backend_rpc_add_server_fn(server.c_str());
...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, int prio) {
            if (prio < LM_GGML_SCHED_PRIO_LOW || prio > LM_GGML_SCHED_PRIO_REALTIME) {
                throw std::invalid_argument("invalid value");
            }
    ...
    add_opt(common_arg(
        ...
        string_format(
            ...
            lm_ggml_type_name(params.cache_type_k)
        ),
        [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg(
        ...
        string_format(
            ...
            "(default: %s)",
            get_all_kv_cache_types().c_str(),
            lm_ggml_type_name(params.cache_type_v)
        ),
        [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, int value) {
            ...
            for (int i = 0; i < value; ++i) {
                ...
                params.tensor_buft_overrides.push_back({buft_overrides.back().c_str(), lm_ggml_backend_cpu_buffer_type()});
            }
        }
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, int value) {
            ...
            for (int i = 0; i < value; ++i) {
                ...
                params.speculative.tensor_buft_overrides.push_back({buft_overrides_draft.back().c_str(), lm_ggml_backend_cpu_buffer_type()});
            }
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}).set_env("LLAMA_ARG_N_CPU_MOE_DRAFT"));
    LM_GGML_ASSERT(params.n_gpu_layers < 0); // string_format would need to be extended for a default >= 0
    add_opt(common_arg(
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, int prio) {
            ...
            params.speculative.cpuparams.priority = (enum lm_ggml_sched_priority) prio;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
        {"--poll-draft"}, "<0|1>",
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, int prio) {
            ...
            params.speculative.cpuparams_batch.priority = (enum lm_ggml_sched_priority) prio;
        }
    ).set_examples({LLAMA_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
    ...
    LM_GGML_ASSERT(params.speculative.n_gpu_layers < 0); // string_format would need to be extended for a default >= 0
    add_opt(common_arg(
    ...
    add_opt(common_arg(
        ...
        string_format(
            ...
            "(default: %s)",
            get_all_kv_cache_types().c_str(),
            lm_ggml_type_name(params.speculative.cache_type_k)
        ),
        [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg(
        ...
        string_format(
            ...
            "(default: %s)",
            get_all_kv_cache_types().c_str(),
            lm_ggml_type_name(params.speculative.cache_type_v)
        ),
        [](common_params & params, const std::string & value) {
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & name) {
            ...
            if (params.optimizer == LM_GGML_OPT_OPTIMIZER_TYPE_COUNT) {
