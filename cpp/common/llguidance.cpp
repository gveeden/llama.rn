...
static size_t llama_sampler_llg_tokenize_fn(const void * user_data, const uint8_t * bytes, size_t bytes_len,
    ...
    try {
        ...
        LM_GGML_ABORT("llama_tokenize failed: %s\n", e.what());
    }
    if (r < 0) {
        return -r;
    }
...
static LlgTokenizer * llama_sampler_llg_new_tokenizer(const llama_vocab * vocab) {
    ...
    for (size_t i = 0; i < vocab_size; i++) {
        ...
        if (token_bytes_size - offset < max_token) {
            LM_GGML_ABORT("token_bytes buffer too small\n");
        }
        ...
        if (size < 0) {
            LM_GGML_ABORT("llama_detokenize failed\n");
        }
        if (size == 0) {
...
llama_sampler * llama_sampler_init_llg(const llama_vocab * vocab, const char * grammar_kind,
    ...
    if (grammar_kind != nullptr && grammar_kind[0] != '\0') {
        ...
        *ctx           = {
        ...
        if (ctx->grammar) {
            LM_GGML_ASSERT(((size_t) llama_vocab_n_tokens(vocab) + 31) / 32 * 4 ==
