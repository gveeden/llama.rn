...
struct tensor_quantization {
    std::string name;
    lm_ggml_type quant = LM_GGML_TYPE_COUNT;
};
...
static void llama_tensor_dequantize_impl(
    lm_ggml_tensor * tensor, std::vector<no_init<float>> & output, std::vector<std::thread> & workers,
...
static lm_ggml_type llama_tensor_get_type(quantize_state_impl & qs, lm_ggml_type new_type, const lm_ggml_tensor * tensor, llama_ftype ftype) {
    ...
    if (name == tn(LLM_TENSOR_OUTPUT, "weight") || (!qs.has_output && name == tn(LLM_TENSOR_TOKEN_EMBD, "weight"))) {
        if (qs.params->output_tensor_type < LM_GGML_TYPE_COUNT) {
            ...
            const int64_t qk_k = lm_ggml_blck_size(new_type);
...
static size_t llama_tensor_quantize_impl(enum lm_ggml_type new_type, const float * f32_data, void * new_data, const int64_t chunk_size, int64_t nrows, int64_t n_per_row, const float * imatrix, std::vector<std::thread> & workers, const int nthread) {
    if (nthread < 2) {
        // single-thread
...
static bool tensor_type_requires_imatrix(const lm_ggml_tensor * t, const lm_ggml_type dst_type, const llama_ftype ftype) {
    return (
        dst_type == LM_GGML_TYPE_IQ2_XXS || dst_type == LM_GGML_TYPE_IQ2_XS ||
        dst_type == LM_GGML_TYPE_IQ3_XXS || dst_type == LM_GGML_TYPE_IQ1_S  ||
        dst_type == LM_GGML_TYPE_IQ2_S   || dst_type == LM_GGML_TYPE_IQ1_M  ||
        (   // Q2_K_S is the worst k-quant type - only allow it without imatrix for token embeddings
            dst_type == LM_GGML_TYPE_Q2_K && ftype == LLAMA_FTYPE_MOSTLY_Q2_K_S && strcmp(t->name, "token_embd.weight") != 0
        )
    );
}
...
static void llama_model_quantize_impl(const std::string & fname_inp, const std::string & fname_out, const llama_model_quantize_params * params) {
    ...
    switch (params->ftype) {
        case LLAMA_FTYPE_MOSTLY_Q1_0: default_type = LM_GGML_TYPE_Q1_0; break;
        case LLAMA_FTYPE_MOSTLY_Q1_0_g128: default_type = LM_GGML_TYPE_Q1_0_g128; break;
        ...
        case LLAMA_FTYPE_MOSTLY_MXFP4_MOE: default_type = LM_GGML_TYPE_MXFP4; break;
    ...
    if (params->kv_overrides) {
        ...
        for (const auto & o : overrides) {
            ...
            if (o.tag == LLAMA_KV_OVERRIDE_TYPE_FLOAT) {
                ...
                lm_gguf_set_val_u32(ctx_out.get(), o.key, (uint32_t)std::abs(o.val_i64));
    ...
    for (const auto & it : ml.weights_map) {
        ...
        if (remapped_name != it.first) {
            lm_ggml_set_name(it.second.tensor, remapped_name.c_str());
            LLAMA_LOG_DEBUG("%s: tensor %s remapped to %s\n", __func__, it.first.c_str(), lm_ggml_get_name(it.second.tensor));
        }
        tensors.push_back(&it.second);
    }
    if (!prune_list.empty()) {
        lm_gguf_set_val_u32(ctx_out.get(), ml.llm_kv(LLM_KV_BLOCK_COUNT).c_str(), blk_id);
    }
    ...
    for (const auto * it : tensors) {
        ...
        lm_ggml_tensor * tensor = it->tensor;
        if (!ctx_outs[i_split]) {
    ...
    if (n_split > 1) {
        for (size_t i = 0; i < ctx_outs.size(); ++i) {
            ...
            lm_gguf_set_val_i32(ctx_outs[i].get(), ml.llm_kv(LLM_KV_SPLIT_TENSORS_COUNT).c_str(), (int32_t)tensors.size());
        }
    }
    ...
    for (const auto * it : tensors) {
        ...
        lm_ggml_tensor * tensor = weight.tensor;
        ...
        if (quantize) {
            ...
            if (!params->pure && lm_ggml_is_quantized(default_type)) {
                ...
                if (params->tensor_types) {
                    ...
                    for (const auto & [tname, qtype] : tensor_types) {
                        if (std::regex pattern(tname); std::regex_search(tensor_name, pattern)) {
                            if  (qtype != new_type) {
                                LLAMA_LOG_WARN("(manual override: %s -> %s) ", lm_ggml_type_name(new_type), lm_ggml_type_name(qtype));
                ...
                // incompatible tensor shapes are handled here - fallback to a compatible type
                {
                    ...
                    if (nx % qk_k != 0) {
                        LLAMA_LOG_WARN("\n\n%s : tensor cols %" PRId64 " x %" PRId64 " are not divisible by %" PRId64 ", required for %s", __func__, nx, ny, qk_k, lm_ggml_type_name(new_type));
        ...
        if (params->dry_run) {
            ...
            if (quantize) {
                new_size = lm_ggml_nrows(tensor) * lm_ggml_row_size(new_type, tensor->ne[0]);
...
#if 0
                    if (new_type == LM_GGML_TYPE_MXFP4) {
                        auto * x = f32_data_03;

                        //LLAMA_LOG_INFO("nrows = %d, n_per_row = %d\n", nrows, n_per_row);
                        std::vector<float> deq(nrows*n_per_row);
                        const lm_ggml_type_traits * qtype = lm_ggml_get_type_traits(new_type);
            ...
            LM_GGML_ASSERT(lm_gguf_get_tensor_size(ctx_outs[cur_split].get(), lm_gguf_find_tensor(ctx_outs[cur_split].get(), name.c_str())) == new_size);
            lm_gguf_set_tensor_data(ctx_outs[cur_split].get(), name.c_str(), new_data);
