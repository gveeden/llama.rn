...
void quantize_row_q8_1(const float * LM_GGML_RESTRICT x, void * LM_GGML_RESTRICT vy, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        y[i].s = LM_GGML_CPU_FP32_TO_FP16(d * vec_extract(accv, 0));
    }
...
#pragma GCC unroll 8
    for (; ib < nb; ++ib) {
        ...
        vector float vxd = vec_splats(LM_GGML_CPU_FP16_TO_FP32(x[ib].d));
        vector float vyd = vec_splats(LM_GGML_CPU_FP16_TO_FP32(y[ib].d));
...
#pragma GCC unroll 4
    for (; ib < nb; ++ib) {
        ...
        vector float vxmin = vec_splats(LM_GGML_CPU_FP16_TO_FP32(x[ib].m));
        vector float vys = {LM_GGML_CPU_FP16_TO_FP32(y[ib].s), 0.0f, 0.0f, 0.0f};
...
#pragma GCC unroll 8
    for (; ib < nb; ++ib) {
        ...
        vector float vyd = vec_splats(LM_GGML_CPU_FP16_TO_FP32(y[ib].d) *
                                      LM_GGML_E8M0_TO_FP32_HALF(x[ib].e));
...
#pragma GCC unroll 4
    for (; ib < nb; ++ib) {
        ...
        vector float vxmin = vec_splats(LM_GGML_CPU_FP16_TO_FP32(x[ib].m));
        vector float vys = {LM_GGML_CPU_FP16_TO_FP32(y[ib].s), 0.f, 0.f, 0.f};
        vsumf0 = vec_madd(vxmin, vys, vsumf0);
...
void lm_ggml_vec_dot_q2_K_q8_K(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, size_t bx, const void * LM_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int i = 0; i < nb; ++i) {
        vector float vxd = vec_splats(LM_GGML_CPU_FP16_TO_FP32(x[i].d));
        vector float vyd = vec_splats(y[i].d);
        vector float vd = vec_mul(vxd, vyd);

        vector float vxmin = vec_splats(LM_GGML_CPU_FP16_TO_FP32(x[i].dmin));
...
void lm_ggml_vec_dot_q6_K_q8_K(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, size_t bx, const void * LM_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const int8_t  * LM_GGML_RESTRICT qs = x[i].scales;
        const int8_t  * LM_GGML_RESTRICT q8 = y[i].qs;
...
void lm_ggml_vec_dot_iq3_xxs_q8_K(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, size_t bx, const void * LM_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const uint8_t * LM_GGML_RESTRICT q3 = x[i].qs;
        const uint32_t * LM_GGML_RESTRICT signs = (const uint32_t *)(x[i].qs + QK_K/4);
        const int8_t  * LM_GGML_RESTRICT q8 = y[i].qs;
...
void lm_ggml_vec_dot_iq1_s_q8_K(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, size_t bx, const void * LM_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const uint8_t  * LM_GGML_RESTRICT q1 = x[i].qs;
        const uint16_t * LM_GGML_RESTRICT qh = x[i].qh;
        const int8_t   * LM_GGML_RESTRICT q8 = y[i].qs;
        const int16_t  * LM_GGML_RESTRICT qs = y[i].bsums;
...
void lm_ggml_vec_dot_iq4_xs_q8_K(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, size_t bx, const void * LM_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int ibl = 0; ibl < nb; ++ibl) {
        ...
        vector float vxd = vec_splats(LM_GGML_CPU_FP16_TO_FP32(x[ibl].d));
        ...
        const uint8_t * LM_GGML_RESTRICT sc = x[ibl].scales_l;
