...
void quantize_row_q8_1(const float * LM_GGML_RESTRICT x, void * LM_GGML_RESTRICT vy, int64_t k) {
    ...
    for (int i = 0; i < nb; i++) {
        ...
        y[i].s = LM_GGML_CPU_FP32_TO_FP16(d * (acc[0] + acc[1] + acc[2] + acc[3]));
    }
#else
    LM_GGML_UNUSED(nb);
    // scalar
...
void lm_ggml_vec_dot_q4_0_q8_0(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, size_t bx, const void * LM_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (; ib < nb; ++ib) {
        ...
        const float32x4_t v_d = vec_splats(LM_GGML_CPU_FP16_TO_FP32(x[ib].d) * LM_GGML_CPU_FP16_TO_FP32(y[ib].d));
    ...
    *s = sumf;
    ...
    UNUSED(ib);
    UNUSED(sumf);
    lm_ggml_vec_dot_q4_0_q8_0_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}
...
#pragma GCC unroll 4
    for (; ib < nb; ++ib) {
        ...
        const int32x4_t v_xy_ = lm_ggml_vec_dot(lm_ggml_vec_dot(vec_splats(0), v_xl, v_yl), v_xh, v_yh);
    ...
    *s = sumf;
    ...
    UNUSED(ib);
    UNUSED(sumf);
    lm_ggml_vec_dot_q4_1_q8_1_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}
...
void lm_ggml_vec_dot_mxfp4_q8_0(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, size_t bx, const void * LM_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    #pragma GCC unroll 8
    for (; ib + 1 < nb; ib += 2) {
        const block_mxfp4 * LM_GGML_RESTRICT x0 = &x[ib + 0];
        const block_mxfp4 * LM_GGML_RESTRICT x1 = &x[ib + 1];
        ...
        const int32x4_t v_xy0 = lm_ggml_vec_dot(lm_ggml_vec_dot(vec_splats(0), v_x0l, v_y0l), v_x0h, v_y0h);
        const int32x4_t v_xy1 = lm_ggml_vec_dot(lm_ggml_vec_dot(vec_splats(0), v_x1l, v_y1l), v_x1h, v_y1h);

        const float32x4_t v_xy0f = vec_float(v_xy0);
        const float32x4_t v_xy1f = vec_float(v_xy1);

        const float32x4_t v_d0 = vec_splats(LM_GGML_E8M0_TO_FP32_HALF(x0->e) * LM_GGML_CPU_FP16_TO_FP32(y0->d));
        const float32x4_t v_d1 = vec_splats(LM_GGML_E8M0_TO_FP32_HALF(x1->e) * LM_GGML_CPU_FP16_TO_FP32(y1->d));
    ...
    for (; ib < nb; ++ib) {
        ...
        const int32x4_t v_xy = lm_ggml_vec_dot(lm_ggml_vec_dot(vec_splats(0), v_xl, v_yl), v_xh, v_yh);
        const float32x4_t v_xyf = vec_float(v_xy);

        const float32x4_t v_d = vec_splats(LM_GGML_E8M0_TO_FP32_HALF(x0->e) * LM_GGML_CPU_FP16_TO_FP32(y0->d));
    ...
    *s = sumf;
    ...
    UNUSED(ib);
    UNUSED(sumf);
    lm_ggml_vec_dot_mxfp4_q8_0_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}
...
void lm_ggml_vec_dot_q5_0_q8_0(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, size_t bx, const void * LM_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    #pragma GCC unroll 4
    for (; ib + 1 < nb; ib += 2) {
        const block_q5_0 * LM_GGML_RESTRICT x0 = &x[ib + 0];
        ...
        const int32x4_t v_xy0 = lm_ggml_vec_dot(lm_ggml_vec_dot(vec_splats(0), v_x0lf, v_y0l), v_x0hf, v_y0h);
        const int32x4_t v_xy1 = lm_ggml_vec_dot(lm_ggml_vec_dot(vec_splats(0), v_x1lf, v_y1l), v_x1hf, v_y1h);

        const float32x4_t v_xy0f = vec_float(v_xy0);
        const float32x4_t v_xy1f = vec_float(v_xy1);

        const float32x4_t v_d0 = vec_splats(LM_GGML_CPU_FP16_TO_FP32(x0->d) * LM_GGML_CPU_FP16_TO_FP32(y0->d));
        const float32x4_t v_d1 = vec_splats(LM_GGML_CPU_FP16_TO_FP32(x1->d) * LM_GGML_CPU_FP16_TO_FP32(y1->d));
    ...
    for (; ib < nb; ++ib) {
        ...
        const int32x4_t v_xy = lm_ggml_vec_dot(lm_ggml_vec_dot(vec_splats(0), v_xlf, v_yl), v_xhf, v_yh);
        const float32x4_t v_xyf = vec_float(v_xy);

        const float32x4_t v_d = vec_splats(LM_GGML_CPU_FP16_TO_FP32(x0->d) * LM_GGML_CPU_FP16_TO_FP32(y0->d));
    ...
    *s = sumf;
    ...
    UNUSED(ib);
    UNUSED(sumf);
    lm_ggml_vec_dot_q5_0_q8_0_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}
...
void lm_ggml_vec_dot_q5_1_q8_1(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, size_t bx, const void * LM_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    #pragma GCC unroll 4
    for (; ib + 1 < nb; ib += 2) {
        const block_q5_1 * LM_GGML_RESTRICT x0 = &x[ib + 0];
    ...
    for (; ib < nb; ++ib) {
        ...
        float summs = LM_GGML_CPU_FP16_TO_FP32(x0->m) * LM_GGML_CPU_FP16_TO_FP32(y0->s);
    ...
    *s = sumf;
    ...
    UNUSED(ib);
    UNUSED(sumf);
    lm_ggml_vec_dot_q5_1_q8_1_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}
...
#pragma GCC unroll 8
    ...
    *s = sumf;
    ...
    UNUSED(ib);
    UNUSED(sumf);
    lm_ggml_vec_dot_q8_0_q8_0_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}
...
void lm_ggml_vec_dot_q4_K_q8_K(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, size_t bx, const void * LM_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const uint8_t * LM_GGML_RESTRICT x0 = x[i].qs;
        const int8_t  * LM_GGML_RESTRICT y0 = y[i].qs;
        ...
        for (int j = 0; j < QK_K/64; ++j) {
            ...
            const int32x4_t p1 = lm_ggml_vec_dot(lm_ggml_vec_dot(v_z, v_xl[0], v_y[0]), v_xl[1], v_y[1]);
            ...
            const int32x4_t p2 = lm_ggml_vec_dot(lm_ggml_vec_dot(v_z, v_xl[0], v_y[0]), v_xl[1], v_y[1]);
...
void lm_ggml_vec_dot_q5_K_q8_K(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, size_t bx, const void * LM_GGML_RESTRICT vy,  size_t by, int nrc) {
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const uint8_t * scales = (const uint8_t *)utmp;
        const uint8_t * LM_GGML_RESTRICT x0l = x[i].qs;
        const uint8_t * LM_GGML_RESTRICT x0h = x[i].qh;
        const int8_t  * LM_GGML_RESTRICT y0 = y[i].qs;
        ...
        for (int j = 0; j < QK_K/64; ++j) {
            ...
            int32x4_t sumi0 = lm_ggml_vec_dot(lm_ggml_vec_dot(v_z, q5b[0], v_y[0]), q5b[1], v_y[1]);
            int32x4_t sumi1 = lm_ggml_vec_dot(lm_ggml_vec_dot(v_z, q5b[2], v_y[2]), q5b[3], v_y[3]);
...
void lm_ggml_vec_dot_q6_K_q8_K(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, size_t bx, const void * LM_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        const uint8_t * LM_GGML_RESTRICT x0l = x[i].ql;
        const uint8_t * LM_GGML_RESTRICT x0h = x[i].qh;
        const int8_t  * LM_GGML_RESTRICT y0 = y[i].qs;
        ...
        for (int j = 0; j < QK_K/128; ++j) {
            ...
            int32x4_t summs0 = lm_ggml_vec_dot(v_z, q6b[0], v_y[0]);
            int32x4_t summs1 = lm_ggml_vec_dot(v_z, q6b[1], v_y[1]);
            int32x4_t summs2 = lm_ggml_vec_dot(v_z, q6b[2], v_y[2]);
            int32x4_t summs3 = lm_ggml_vec_dot(v_z, q6b[3], v_y[3]);
            ...
            summs0 = lm_ggml_vec_dot(v_z, q6b[0], v_y[0]);
            summs1 = lm_ggml_vec_dot(v_z, q6b[1], v_y[1]);
            summs2 = lm_ggml_vec_dot(v_z, q6b[2], v_y[2]);
            summs3 = lm_ggml_vec_dot(v_z, q6b[3], v_y[3]);
...
//            const int32x4_t p1 = lm_ggml_vec_dot(lm_ggml_vec_dot(vec_splat_s32(0), q2u0, q8b0), q2u1, q8b1);
//            const int32x4_t p2 = lm_ggml_vec_dot(lm_ggml_vec_dot(vec_splat_s32(0), q2u2, q8b2), q2u3, q8b3);
...
void lm_ggml_vec_dot_iq4_nl_q8_0(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, size_t bx, const void * LM_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (; ib < nb; ++ib) {
        const block_iq4_nl * LM_GGML_RESTRICT x0 = &x[ib];
        const block_q8_0   * LM_GGML_RESTRICT y0 = &y[ib];
        ...
        sumf += LM_GGML_CPU_FP16_TO_FP32(x0->d) * LM_GGML_CPU_FP16_TO_FP32(y0->d) * vec_hsum_i32x4(v_xy);
    }
    ...
    *s = sumf;
    ...
    UNUSED(ib);
    UNUSED(sumf);
    lm_ggml_vec_dot_iq4_nl_q8_0_generic(n, s, bs, vx, bx, vy, by, nrc);
#endif
}
...
void lm_ggml_vec_dot_iq4_xs_q8_K(int n, float * LM_GGML_RESTRICT s, size_t bs, const void * LM_GGML_RESTRICT vx, size_t bx, const void * LM_GGML_RESTRICT vy, size_t by, int nrc) {
    ...
    for (int ibl = 0; ibl < nb; ++ibl) {
        const uint8_t * LM_GGML_RESTRICT q4 = x[ibl].qs;
        const int8_t  * LM_GGML_RESTRICT q8 = y[ibl].qs;
        ...
        for (int ib = 0; ib < QK_K/64; ++ib) {
            ...
            int32x4_t vsumi0 = lm_ggml_vec_dot(lm_ggml_vec_dot(vec_splats(0), v_x0l, v_y0), v_x0h, v_y1);
            int32x4_t vsumi1 = lm_ggml_vec_dot(lm_ggml_vec_dot(vec_splats(0), v_x1l, v_y2), v_x1h, v_y3);
