...
static void init_kleidiai_context(void) {
    ...
    if (!initialized) {
        initialized = true;
        const char *env_var = getenv("LM_GGML_KLEIDIAI_SME");
        int sme_enabled = 0;
        ...
        if (sme_enabled != 0) {
            ctx.features |= lm_ggml_cpu_has_sme() ? CPU_FEATURE_SME : CPU_FEATURE_NONE;
        }
...
#ifndef NDEBUG
        if (ctx.kernels_q4) {
            LM_GGML_LOG_DEBUG("kleidiai: using q4 kernel with CPU feature %s\n", cpu_feature_to_string(ctx.kernels_q4->required_cpu));
        }
        if (ctx.kernels_q8) {
            LM_GGML_LOG_DEBUG("kleidiai: using q8 kernel with CPU feature %s\n", cpu_feature_to_string(ctx.kernels_q8->required_cpu));
        }
#endif
    }
...
class tensor_traits : public ggml::cpu::tensor_traits {
    bool work_size(int /* n_threads */, const struct lm_ggml_tensor * op, size_t & size) override {
        ...
        lm_ggml_kleidiai_kernels *kernels = lm_ggml_kleidiai_select_kernels(ctx.features, op);
        if (!kernels) {
            return false;
        }
    ...
    bool compute_forward(struct lm_ggml_compute_params * params, struct lm_ggml_tensor * dst) override {
        if (dst->op == LM_GGML_OP_MUL_MAT) {
            if (dst->src[0]->type == LM_GGML_TYPE_Q4_0) {
                return compute_forward_q4_0(params, dst);
            } else if (dst->src[0]->type == LM_GGML_TYPE_Q8_0) {
    ...
    bool compute_forward_q4_0(struct lm_ggml_compute_params * params, struct lm_ggml_tensor * dst) {
        LM_GGML_ASSERT(dst->src[0]->type == LM_GGML_TYPE_Q4_0);
    ...
    bool compute_forward_q8_0(struct lm_ggml_compute_params * params, struct lm_ggml_tensor * dst) {
        LM_GGML_ASSERT(dst->src[0]->type == LM_GGML_TYPE_Q8_0);
...
public:
    int repack(struct lm_ggml_tensor * tensor, const void * data, size_t data_size) {
        ...
        if (tensor->type == LM_GGML_TYPE_Q4_0) {
            ...
            return 0;
        } else if (tensor->type == LM_GGML_TYPE_Q8_0) {
            if (!ctx.kernels_q8) {
                return -1;
            }
...
static size_t lm_ggml_backend_cpu_kleidiai_buffer_type_get_alloc_size(lm_ggml_backend_buffer_type_t buft, const struct lm_ggml_tensor * tensor) {
    ...
    if (tensor->type == LM_GGML_TYPE_Q4_0) {
        LM_GGML_ASSERT(ctx.kernels_q4);
        kernels = ctx.kernels_q4;
        block_len = QK4_0;
    } else if (tensor->type == LM_GGML_TYPE_Q8_0) {
        LM_GGML_ASSERT(ctx.kernels_q8);
        kernels = ctx.kernels_q8;
        block_len = QK8_0;
    } else {
        return 0;
    ...
    const size_t raw     = lm_ggml_nbytes(tensor);
...
class extra_buffer_type : ggml::cpu::extra_buffer_type {
    bool supports_op(lm_ggml_backend_dev_t, const struct lm_ggml_tensor * op) override {
        if ((op->op == LM_GGML_OP_MUL_MAT || op->op == LM_GGML_OP_GET_ROWS) &&
            ...
            if (((op->src[0]->type == LM_GGML_TYPE_Q4_0) ? ctx.kernels_q4 : ctx.kernels_q8) == nullptr) {
                return false;
            }
            ...
            if ((op->src[1]->type == LM_GGML_TYPE_F32 || op->src[1]->type == LM_GGML_TYPE_I32) &&
                lm_ggml_ne(op->src[1], 2) == 1 && lm_ggml_ne(op->src[1], 3) == 1) {
                return true;
            }
        }
        return false;
    ...
    ggml::cpu::tensor_traits * get_tensor_traits(const struct lm_ggml_tensor * op) override {
        if (op->op == LM_GGML_OP_MUL_MAT || op->op == LM_GGML_OP_GET_ROWS) {
            ...
            else if (lm_ggml_kleidiai_select_kernels(ctx.features, op) && op->src[1]->ne[1] > 1) {
